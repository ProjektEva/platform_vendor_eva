...
func init() {
	android.RegisterModuleType("eva_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = evaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = evaExpandVariables(ctx, dep_file)
	...
	cmd := evaExpandVariables(ctx, String(g.properties.Cmd))
