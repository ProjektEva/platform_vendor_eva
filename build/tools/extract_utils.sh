...
function setup_vendor() {
    ...
    export EVA_ROOT="$3"
    if [ ! -d "$EVA_ROOT" ]; then
        echo "\$EVA_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$EVA_ROOT/$OUTDIR" ]; then
        mkdir -p "$EVA_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$EVA_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$EVA_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$EVA_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$EVA_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$EVA_ROOT"/prebuilts/tools-custom/common/smali/baksmali.jar
        export SMALIJAR="$EVA_ROOT"/prebuilts/tools-custom/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$EVA_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$EVA_ROOT"/prebuilts/tools-custom/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$EVA_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$EVA_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$EVA_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$EVA_ROOT"/vendor/eva/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$EVA_ROOT"/"$OUTDIR"/radio
