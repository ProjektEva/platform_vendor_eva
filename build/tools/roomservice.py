...
eva_local_manifest = ".repo/local_manifests/eva.xml"
eva_default_revision = "androidX"
eva_dependencies = "eva.dependencies"
org_manifest = "eva-devices"  # leave empty if org is provided in manifest
org_display = "ProjektEva"  # needed for displaying
...
def get_from_manifest(device_name):
    if os.path.exists(eva_local_manifest):
        man = load_manifest(eva_local_manifest)
...
def is_in_manifest(project_path):
    for man in (eva_local_manifest, default_manifest):
...
def add_to_manifest(repos, fallback_branch=None):
    lm = load_manifest(eva_local_manifest)
	...
	if 'branch' in repo:
	    ...
	    repo_branch=eva_default_revision
	if 'remote' in repo:
...
def fetch_dependencies(repo_path, fallback_branch=None):
    ...
    dep_p = '/'.join((repo_path, eva_dependencies))
...
def detect_revision(repo):
    ...
    if has_branch(result, eva_default_revision):
