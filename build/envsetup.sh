function __print_eva_functions_help() {
cat <<EOF
...
- evaremote:      Add git remote for matching eva repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for eva Github.
...
- repopick:        Utility to fetch changes from eva Gerrit.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/eva-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.eva.device | grep -q "$EVA_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $EVA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function evaremote()
{
    ...
    git remote add eva https://android.googlesource.com/$PFX$PROJECT
...
function githubremote()
{
    ...
    git remote add github https://github.com/eva/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop ro.eva.device | grep -q "$EVA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.eva.device | grep -q "$EVA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
